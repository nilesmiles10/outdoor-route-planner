...
// Client-hub. `initialRows`/`initialFeatured` komen van de server-wrapper zodat
// de publieke routes in de geleverde HTML staan (crawlbaar). De filters,
// sortering en geolocatie blijven volledig client-side — die zijn per bezoeker
// en horen niet in een gecachte server-render.
export default function DiscoverClient({
  ...
  // "geen routes gevonden"-tekst (misleidend — ze laden nog). Met server-data
  // is er niets om op te wachten, dus dan meteen uit.
  const [loading, setLoading] = useState(initialRows.length === 0);
  const [featured, setFeatured] = useState<Row[]>(initialFeatured);
