...
| Loop / round-trip route planner | "circular walk planner", "loop route generator", "rondje maken", "rondje van X km" | `/[locale]/loop-route-planner` | Feature | P2 | DONE | Discovered opportunity. Built on the real round-trip generator (GEN-107 loopVias: set distance → loop of ~that length + variations). Inbound from running + hiking. |
